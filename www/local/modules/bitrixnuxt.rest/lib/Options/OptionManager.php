...
use Bitrixnuxt\Rest\Options\OptionsType\{OptionTypeInt, OptionTypeString, OptionTypeCheckbox};
use Bitrixnuxt\Rest\Options\OptionList;
use Bitrixnuxt\Rest\Main\Logger;
...
final Class OptionManager
{
    ...
    public const MODULE_ID = 'bitrixnuxt.rest';
    ...
    public function getFullCustomOptions(): array
    {
        ...
        foreach(GetModuleEvents(BITRIX_NUXT_MODULE_FUNCTIONS, 'OnCreateBaseSettingsList', true) as $arEvent){
        ...
        foreach ($SettingsList as $optionKey => $optionValue) {
            try {
                ...
                if( !array_key_exists($optionValueType, self::OPTION_TYPES) ) throw new Exception(Loc::getMessage('ERROR_NOT_SUPPORT_PROPERTY_TYPE', [
                ...
                $className = "Bitrixnuxt\\Rest\\Options\\OptionsType\\OptionType". ucfirst($optionValueType);
    ...
    public function getOptionsList(): ?array
    {
        foreach ($this->getFullOptions() as $optionKey => $optionValue) {
            try {
                ...
                $className = "Bitrixnuxt\\Rest\\Options\\OptionsType\\OptionType". ucfirst($optionValue['TYPE']);
    ...
    {
        try {
            $className = "Bitrixnuxt\\Rest\\Options\\OptionsType\\OptionType" . ucfirst($type);
    ...
    {
        try { 
            if( !in_array($type, self::OPTION_TYPES) ) throw new Exception(Loc::getMessage('ERROR_NOT_SUPPORT_PROPERTY_TYPE', [
                '#TYPE#' => $type,
                '#CODE#' => $code,
            ]));
